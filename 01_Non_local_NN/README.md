...
$f(x_i, x_j)=e^{θ(x_i)^Tϕ(𝐱_j)}.$ (3)

$𝒞(x)=\sum\limits_{\forall j} f(x_i, x_j)$ (4)

$\mathbf{y_i} = \frac{1}{\mathcal{C}(\mathbf{x})} \sum\limits_{\forall j} f(\mathbf{x}_i, \mathbf{x}_j)g(\mathbf{x}_j)$ (5)
...
g가 높고 f가 높으면 확실한 특징이 있고 이것이 나와 관련 있다는 의미이다.

g가 높고 f가 낮으면 확실한 특징은 있지만 이것이 나와 관련은 없다는 의미이다.

g가 낮고 f가 높으면 확실한 특징은 없지만 나와 관련이 있다는 의미이다.

g가 낮고 f가 낮으면 확실한 특징도 없고 나와 관련도 없다는 의미이다.
